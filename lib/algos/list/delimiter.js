...
class Delimiter extends Extension {
    ...
    genMDParamsV1() {
        ...
        const params = [{}, {}];
        if (this.prefix) {
            params[0].gte = DbPrefixes.Master + this.prefix;
            params[0].lt = DbPrefixes.Master + inc(this.prefix);
            params[1].gte = DbPrefixes.Version + this.prefix;
            params[1].lt = DbPrefixes.Version + inc(this.prefix);
        } else {
        ...
        const startVal = this[this.continueMarker] || this[this.startMarker];
        if (startVal && params[1].gte <= DbPrefixes.Version + startVal) {
            delete params[0].gte;
            delete params[1].gte;
            params[0].gt = DbPrefixes.Master + inc(startVal + VID_SEP);
            params[1].gt = DbPrefixes.Version + inc(startVal + VID_SEP);
        }
        return params;
    }
