...
class MongoClientInterface {
    ...
    getBucketAttributes(bucketName, log, cb) {
        const m = this.getCollection(METASTORE);
        console.time('getBucketAttributes (findOne)');
        m.findOne({
            _id: bucketName,
        }, {}, (err, doc) => {
            console.timeEnd('getBucketAttributes (findOne)'); // 1 - 500ms
            if (err) {
                log.error(
    ...
    getBucketAndObject(bucketName, objName, params, log, cb) {
        console.time('getBucketAndObject (getBucketAttributes)');
        this.getBucketAttributes(bucketName, log, (err, bucket) => {
            ...
            console.timeEnd('getBucketAndObject (getBucketAttributes)');
            console.time('getBucketAndObject (getObject)');
            console.time('getBucketAndObject (getBucketAttributes time before fn)');
            this.getObject(bucketName, objName, params, log, (err, obj) => {
                console.timeEnd('getBucketAndObject (getObject)') // 40-150ms
                console.timeEnd('getBucketAndObject (getBucketAttributes time after fn)'); // 3ms
                if (err) {
                    if (err.is.NoSuchKey) {
                        return cb(null,
                            {
    ...
    getObject(bucketName, objName, params, log, cb) {
        console.timeEnd('getBucketAndObject (getBucketAttributes time before fn)'); // 1ms
        const c = this.getCollection(bucketName);
        let key;
        async.waterfall([
            ...
            (vFormat, next) => {
                ...
                console.time('getObject (findOne)');
                c.findOne({
                    _id: key,
                }, {}, (err, doc) => {
                    console.timeEnd('getObject (findOne)'); // 1 - 200 ms
                    if (err) {
            ...
            (vFormat, doc, next) => {
                console.time('getObject (wtaerfall callback)');
                ...
                console.timeEnd('getObject (wtaerfall callback)'); // 1 - 150 ms
                MongoUtils.unserialize(doc.value);
                return next(null, doc.value);
            },
        ], (err, data) => {
            console.time('getBucketAndObject (getBucketAttributes time after fn)');
            process.nextTick(() => {
                return cb(err, data);
            });
        });
