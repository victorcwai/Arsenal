...
class MongoClientInterface {
    ...
    getBucketAttributes(bucketName, log, cb) {
        console.time('heyhey0');
        const m = this.getCollection(METASTORE);
        console.timeEnd('heyhey0'); // <1ms
        log.info('time taken by 2.5');
        console.time('heyhey');
        m.findOne({
            _id: bucketName,
        }, {}, (err, doc) => {
            console.timeEnd('heyhey'); // 1 - 500ms
            log.info('time taken by 5');
            if (err) {
                log.error(
            ...
            console.timeEnd('heyhey2'); // <1ms
            log.info('time taken by 6');
            process.nextTick(() => {
    ...
    getBucketAndObject(bucketName, objName, params, log, cb) {
        ...
        this.getBucketAttributes(bucketName, log, (err, bucket) => {
            ...
            log.debug('time taken by 2', {
                timeTaken: console.timeEnd(objName + '1'), // 20 - 800ms
            })
            console.time('callbaaaaack1ttt');
            console.time('callbaaaaack1');
            this.getObject(bucketName, objName, params, log, (err, obj) => {
                log.info('time taken by 9.5');
                console.timeEnd('callbaaaaack2'); // 3ms
                console.timeEnd('callbaaaaack1ttt') // 20 ms - 2s
                log.debug('time taken by 3')
                if (err) {
                    if (err.is.NoSuchKey) {
                        return cb(null,
                            {
    ...
    getObject(bucketName, objName, params, log, cb) {
        log.info('time taken by 10.5');
        console.timeEnd('callbaaaaack1'); // 1ms
        console.time('heyheym');
        const c = this.getCollection(bucketName);
        console.timeEnd('heyheym'); // <1ms
        log.info('time taken by 3.5');
        let key;
        console.time('heyheymm');

        async.waterfall([
            next => this.getBucketVFormat(bucketName, log, next),
            (vFormat, next) => {
                console.timeEnd('heyheymm'); // <1ms
                log.info('time taken by 4.5');
                console.time('heyheymmm');
                if (params && params.versionId) {
                    key = formatVersionKey(objName, params.versionId, vFormat);
                } else {
                    key = formatMasterKey(objName, vFormat);
                }
                console.timeEnd('heyheymmm'); // < 0.5ms
                log.info('time taken by 5.5', {
                    keyis: key,
                });
                console.time('heyheymmmm');
                c.findOne({
                    _id: key,
                }, {}, (err, doc) => {
                    console.timeEnd('heyheymmmm'); // 1 - 200 ms
                    log.info('time taken by 6.5');
                    if (err) {
            ...
            (vFormat, doc, next) => {
                ...
                console.timeEnd('heyheymmmmcb'); // 1 - 150 ms
                log.info('time taken by 7.5');
                console.time('heyheymmmmcb2');

                MongoUtils.unserialize(doc.value);
                log.info('time taken by 8.5');
                console.timeEnd('heyheymmmmcb2'); // <1ms
                return next(null, doc.value);
            },
        ], (err, data) => {
            console.time('callbaaaaack2');
            process.nextTick(() => {
                return cb(err, data);
            });
        });
    ...
    getObjectMDStats(bucketName, bucketInfo, isTransient, log, callback) {
        ...
        cursor.forEach(
            ...
            err => {
                ...
                console.timeEnd('getObjectMDStats');
