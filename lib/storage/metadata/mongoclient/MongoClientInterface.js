...
class MongoClientInterface {
    ...
    setup(cb) {
        ...
        this.bulked = process.env.USE_BULK && process.env.USE_BULK != '0' ? true : false;
        console.log('CONNEXION', this.mongoUrl, options);
    ...
    getBucketAttributes(bucketName, log, cb) {
        ...
        // console.time(bucketName + rd + 'getBucketAttributes (findOne)');
        // let lastELU = eventLoopUtilization();
        m.findOne({
            _id: bucketName,
        }, {}, (err, doc) => {
            // console.timeEnd(bucketName + rd + 'getBucketAttributes (findOne)'); // 1 - 500ms
            // const tmpELU = eventLoopUtilization();
            // console.log('getBucketAttributes (findOneUtil)', eventLoopUtilization(tmpELU, lastELU).utilization);
            // console.log('getBucketAttributes (findOneActiv)', eventLoopUtilization(tmpELU, lastELU).active);
            if (err) {
                log.error(
            ...
            // console.time(bucketName + rd2 + 'getBucketAttributes (parsing)');
            const bucketMDStr = stringify(doc.value);
            const bucketMD = BucketInfo.deSerialize(bucketMDStr);
            // console.timeEnd(bucketName + rd2 + 'getBucketAttributes (parsing)');
            //process.nextTick(() => {
    ...
    getBucketAndObject(bucketName, objName, params, log, cb) {
        // console.time(bucketName + objName + 'getBucketAndObject (getBucketAttributes)'); // 215ms
        this.getBucketAttributes(bucketName, log, (err, bucket) => {
            ...
            // console.timeEnd(bucketName + objName + 'getBucketAndObject (getBucketAttributes)');
            // console.time(bucketName + objName + 'getBucketAndObject (getObject)');
            // console.time(bucketName + objName + 'getBucketAndObject (getBucketAttributes time before fn)');
            this.getObject(bucketName, objName, params, log, (err, obj) => { // 297ms
                // console.timeEnd(bucketName + objName + 'getBucketAndObject (getObject)') // 40-150ms
                // console.timeEnd(bucketName + objName + 'getBucketAndObject (getBucketAttributes time after fn)'); // 3ms
                if (err) {
                    if (err.is.NoSuchKey) {
                        return cb(null,
                            {
    ...
    putObjectNoVer(c, bucketName, objName, objVal, params, log, cb) {
        ...
        if (!global.c[bucketName]) {
            global.c[bucketName] = [];
            global['off' + bucketName] = 0;
        }
        if (this.bulked) {
            return global.c[bucketName].push([{
                updateOne: {
    ...
    bulkUpdates() {
        ...
        // Every 10ms, bulk write everything we have
        setInterval(() => {
            Object.keys(global.c || {}).forEach(bucketName => {
                this.bulkPutObjectNoVer(bucketName);
            });
        }, process.env.BULK_FREQ || 10);
    }

    async bulkPutObjectNoVer(bucketName) {
        const c = this.getCollection(bucketName);
        const operations = [];
        const cbs = [];

        const toRead = global.c[bucketName].length;
        for (let i = 0; i < toRead; i++) {
            operations.push(global.c[bucketName][i][0]);
            cbs.push(global.c[bucketName][i][1]);
        }
        global['off' + bucketName] = toRead;
        global.c[bucketName].splice(0, toRead);
        // console.log('WRITES:', toRead, bucketName)
        if (operations.length > 0) {
            await c.bulkWrite(operations);
            return cbs.forEach(cb => setImmediate(() => cb()));
        }
    }
    ...
    getObject(bucketName, objName, params, log, cb) {
        // console.timeEnd(bucketName + objName + 'getBucketAndObject (getBucketAttributes time before fn)'); // 1ms
        const c = this.getCollection(bucketName);
        let key;
        async.waterfall([
            ...
            (vFormat, next) => {
                ...
                // console.time(bucketName + objName + 'getObject (findOne)');
                c.findOne({
                    _id: key,
                }, {}, (err, doc) => {
                    // console.timeEnd(bucketName + objName + 'getObject (findOne)'); // 1 - 200 ms
                    if (err) {
            ...
            (vFormat, doc, next) => {
                const rd = Math.random().toString();
                // console.time(rd + 'getObject (wtaerfall callback)');
                ...
                // TODO: check size ? if 0 -> ignore this call
                if (!doc || doc.value.isPHD) {
                    this.getLatestVersion(c, objName, vFormat, log, (err, value) => {
                        ...
                        // console.timeEnd(rd + 'getObject (wtaerfall callback)'); // 1 - 150 ms
            ...
            // console.time(bucketName + objName + 'getBucketAndObject (getBucketAttributes time after fn)');
            //process.nextTick(() => {
                return cb(err, data);
            //});
        });
    ...
    getObjectMDStats(bucketName, bucketInfo, isTransient, log, callback) {
        ...
        const rd = Math.random().toString();
        // console.time(bucketName + rd + 'getObjectMDStats');
        log.info('getObjectMDStats');
        cursor.forEach(
            res => {
            ...
            err => {
                ...
                // console.timeEnd(bucketName + rd + 'getObjectMDStats');
                log.info('getObjectMDStats took');
